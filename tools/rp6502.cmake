...
function(rp6502_xram header regex)
    ...
    if (failed)
        ...
        file(WRITE "${dir}/unread.txt"
            "rp6502_xram(${header}) read no addresses, so every name is zero.\n${output}\n")
        set(unread
            COMMAND "${CMAKE_COMMAND}" -E cat "${dir}/unread.txt"
    ...
    add_custom_command(
        OUTPUT "${dir}/check.stamp"
        DEPENDS "${header_file}" "${dir}/check.c"
        COMMAND "${CMAKE_C_COMPILER}" ${compiler_args} ${flags} -I "${header_dir}"
                -c -o "${dir}/check.o" "${dir}/check.c"
        ${unread}
        COMMAND "${CMAKE_COMMAND}" -E touch "${dir}/check.stamp"
        COMMENT "Checking ${header_name}"
        VERBATIM
    )
    add_custom_target(${target} ALL DEPENDS "${dir}/check.stamp")
